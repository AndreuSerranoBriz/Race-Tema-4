...
```{r}
by_package%>% summarise(median(size))
```
...
```{r}
top_counts <- pack_sum %>% filter(count > quantile(count, probs = 0.99))
...
top_counts_sorted <- top_counts %>% arrange(desc(count))
...
```{r}
top_unique_sorted <- pack_sum %>% arrange(desc(unique))
...
library(tidyr)
library(readr)
datostidyr <- read_csv("data/datostidyr.Rdata")
students <- data.frame(
  grade = c("A", "B", "C", "D", "E"),
  male = c(1, 5, 5, 5, 7),
  female = c(5, 0, 2, 5, 4)
...
students %>%pivot_longer(cols = c(male, female), names_to = "sex", values_to = "count")
...
students2 %>% pivot_longer(cols = -grade, names_to = c("sex", "class"), names_sep = "_", values_to = "count")
...
students3_tidy <- students3 %>%pivot_longer(cols = -name,  names_to = c("class", "exam"),  names_sep = "_",  values_to = "score",  values_drop_na = TRUE) %>%pivot_wider(names_from = exam,  values_from = score)
...
students4 %>%select(id, name, sex) %>%distinct()

students4 %>%select(id, class, midterm, final)
...
all_students <- bind_rows(failed %>% mutate(status = "failed"),passed %>% mutate(status = "passed"))
head(all_students)
...
```{r}
sat_tidy <- sat %>%pivot_longer(cols = -score_range,names_to = c("part", "sex"),names_sep = "_",values_to = "count") %>% filter(sex != "total") %>% mutate(sex = ifelse(sex == "fem", "female", sex))  
...
```{r}
sat_grupo <- sat_tidy %>%group_by(part, sex) %>%summarise(total = sum(count),prop = count / total)
...
```{r}
dt2 <- ymd(c("2014-07-14", "2014-08-20", "2014-11-29"))
head(dt2)
...
```{r}
ahora <- update(ahora, hours = 8, minutes = 34, seconds = 55)
head(ahora)
...
llegada_hk <- with_tz(llegada_nyc, "Asia/Hong_Kong")
print(salida)
print(llegada_nyc)
print(llegada_hk)
